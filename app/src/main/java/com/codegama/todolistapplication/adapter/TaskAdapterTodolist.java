...
public class TaskAdapterTodolist extends RecyclerView.Adapter<TaskAdapterTodolist.TaskViewHolder> {
    ...
    CreateTodolistBottomSheetFragment.setRefreshListener setRefreshListener;

    public TaskAdapterTodolist(TodolistActivity context, List<Todolist> todolistList, CreateTodolistBottomSheetFragment.setRefreshListener setRefreshListener) {
    ...
    @NonNull
    @Override
    public TaskAdapterTodolist.TaskViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
        View view = inflater.inflate(R.layout.item_task, viewGroup, false);
        return new TaskAdapterTodolist.TaskViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull TaskViewHolder holder, int position) {
        Todolist todolist = todolistList.get(position);
        holder.title.setText(todolist.getTaskTitle());
        holder.description.setText(todolist.getTaskDescrption());
        holder.time.setText(todolist.getLastAlarm());
        holder.status.setText(todolist.isComplete() ? "COMPLETED" : "UPCOMING");
        ...
        popupMenu.setOnMenuItemClickListener(item -> {
            switch (item.getItemId()) {
                case R.id.menuDelete:
                    ...
                    alertDialogBuilder.setTitle(R.string.delete_confirmation).setMessage(R.string.sureToDelete).
                            setPositiveButton(R.string.yes, (dialog, which) -> {
                                deleteTaskFromId(todolist.getTaskId(), position);
                            })
                ...
                case R.id.menuUpdate:
                    ...
                    createTodolistBottomSheetFragment.setTodolistId(todolist.getTaskId(), true, context, context);
                    createTodolistBottomSheetFragment.show(context.getSupportFragmentManager(), createTodolistBottomSheetFragment.getTag());
                    break;
                case R.id.menuComplete:
                    ...
                    completeAlertDialog.setTitle(R.string.confirmation).setMessage(R.string.sureToMarkAsComplete).
                            setPositiveButton(R.string.yes, (dialog, which) -> showCompleteDialog(todolist.getTaskId(), position))
                            .setNegativeButton(R.string.no, (dialog, which) -> dialog.cancel()).show();
                    break;
            }
            return false;
    ...
    private void deleteTaskFromId(int taskId, int position) {
        class GetSavedTasks extends AsyncTask<Void, Void, List<Todolist>> {
            ...
            protected List<Todolist> doInBackground(Void... voids) {
                TodolistDatabaseClient.getInstance(context)
                        .getTodolistDatabase()
    ...
    private void removeAtPosition(int position) {
        ...
        notifyItemRangeChanged(position,todolistList.size());
    }
